...
| [3.0.0 Alpha 1](https://github.com/Yifo98/Media-Dock/releases/tag/v3.0.0-alpha.1) | 自愿安装的 Pre-release | 愿意测试全新工作台、任务引擎和诊断流程的用户 |
...
- Windows 解压后运行根目录的 `Launch Media Dock.bat`。
- macOS 解压后运行根目录的 `Launch Media Dock.command`。

标准分享包把核心运行文件放在 `core/`，并在同级 `Media Dock Data/` 保存任务数据、工具、缓存和默认下载。当前包未签名，Windows SmartScreen 或 macOS Gatekeeper 可能在首次运行时提示风险。
...
| [3.0.0 Alpha 1](https://github.com/Yifo98/Media-Dock/releases/tag/v3.0.0-alpha.1) | Opt-in Pre-release | Testers of the new workspace, task engine, and diagnostics flow |
...
- Windows: unzip and run `Launch Media Dock.bat`.
- macOS: unzip and run `Launch Media Dock.command`.

Standard packages keep the runtime inside `core/` and portable data in the sibling `Media Dock Data/` directory. Current packages are unsigned, so first-run platform warnings are expected.
